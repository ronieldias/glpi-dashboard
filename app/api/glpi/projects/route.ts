...
function buildByStatus(projects: GLPIProject[]): ChartDataItem[] {
  ...
  projects.forEach((p) => {
    ...
    if (isCompleted(p)) {
      ...
      label = p.projectstates_id ? String(p.projectstates_id) : "Outro";
    }
...
const PROGRESS_COLOR_ON_TRACK = "#AEC43B";
const PROGRESS_COLOR_NEAR_DEADLINE = "#F59E0B";
const PROGRESS_COLOR_OVERDUE = "#EF4444";
const DEADLINE_WARNING_DAYS = 7;

function getDeadlineColor(p: GLPIProject): string {
  if (!p.plan_end_date) return PROGRESS_COLOR_ON_TRACK;

  const deadline = new Date(p.plan_end_date);
  const now = new Date();
  const diffDays = (deadline.getTime() - now.getTime()) / 86_400_000;

  if (diffDays < 0 && p.percent_done < 100) return PROGRESS_COLOR_OVERDUE;
  if (diffDays <= DEADLINE_WARNING_DAYS) return PROGRESS_COLOR_NEAR_DEADLINE;
  return PROGRESS_COLOR_ON_TRACK;
}
...
function buildList(projects: GLPIProject[]) {
  ...
  return projects.map((p) => {
    ...
    return {
      id: p.id,
      name: p.name,
      status: isCompleted(p)
        ? "Concluído"
        : isOverdue(p)
          ? "Atrasado"
          : "Em andamento",
      manager: p.users_id ? String(p.users_id) : "-",
